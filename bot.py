...
# start OverLoadVC Bor
@app.on_message(filters.command('start') & self_or_contact_filter)
async def start(client, message):
    await message.reply("**Hello, I'm OVerLoadVC Bot ...i am alive..🎵**")
...
async def ping(client, message):
    ...
    await tauk.edit(f'**Pingi pongy speed!**\n> `{m_s} ms`')
...
async def play_track(client, message):
    ...
    if VOICE_CHATS and message.chat.id in VOICE_CHATS:
        text = f'▶️ Playing **{audio.title}** at **{message.chat.title}** by OverLoadVC Bot...'
    else:
        try:
    ...
    await a.edit(f'▶️ Playing **{audio.title}** at **{message.chat.title}** by OverLoadVC Bot...')
...
async def join_voice_chat(client, message):
    ...
    try:
        ...
        await message.reply('Error ⚠️')
        return
    VOICE_CHATS[chat_id] = group_call
    await message.reply('OverLoad Joined the Voice Chat ✅')
