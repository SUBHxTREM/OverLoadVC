...
async def ping(client, message):
    ...
    await tauk.edit(f'**❇️pingi Pongi speed is!❇️**\n> `{m_s} ms`')
...
async def song(client, message):
    ...
    if args.startswith(" "):
        await message.reply("What's the song you are looking for ?🧐")
        return ""
...
async def deezer(_, message):
    if len(message.command) < 2:
        await message.reply_text("What's the song you are looking for ? 🧐")
        return
...
async def play_track(client, message):
    ...
    if VOICE_CHATS and message.chat.id in VOICE_CHATS:
        text = f'▶️ Playing **{audio.title}** here by OverLoadVC Bot...'
    else:
        try:
    ...
    await a.edit(f'▶️ Playing **{audio.title}** here by OverLoadVc Bot...')
...
async def stop_playing(_, message):
    ...
    await message.reply('Stopped Playing ✖️')
...
app.start()
print('>>>  OverLoadVC Bot ACTIVATED😎 /n Thank you for using our Bot❤')
idle()
app.stop()
print('\n>>>  OverLoadVC Bot STOPPED⚠️')
